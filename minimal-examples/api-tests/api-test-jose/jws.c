...
test_jws_none(struct lws_context *context)
{
	...
	if (lws_jws_parse_jose(&jose_alg, map.buf[LJWS_JOSE],
			       map.len[LJWS_JOSE]) < 0) {
...
test_jws_HS256(struct lws_context *context)
{
	...
	/* parse the JOSE header */
	if (lws_jws_parse_jose(&jose_alg, test1, strlen(test1)) < 0) {
		lwsl_err("%s: JOSE parse failed\n", __func__);
		goto bail;
	}

		/* confirm we used the "none" alg as expected from JOSE hdr */
		if (strcmp(jose_alg->alg, "HS256")) {
			lwsl_err("%s: JOSE header has wrong alg\n", __func__);
			goto bail;
		}
	...
	if (lws_genhmac_init(&ctx, jose_alg->hmac_type,
	...
	if (lws_jws_sig_confirm_compact_b64(buf, p - buf, &jwk, context) < 0) {
...
test_jws_RS256(struct lws_context *context)
{
	...
	if (lws_gencrypto_jws_alg_to_definition("RS256", &jose_alg)) {
	...
	if (lws_jws_sig_confirm_compact_b64(rfc7515_rsa_a1,
					    strlen(rfc7515_rsa_a1),
					    &jwk, context) < 0) {
		lwsl_notice("%s: 2.2: confirm rsa sig failed\n", __func__);
		goto bail;
	}

	if (lws_jws_b64_concat_map(rfc7515_rsa_a1, strlen(rfc7515_rsa_a1),
				   &jws.map_b64) != 3) {
		lwsl_notice("%s: lws_jws_b64_concat_map failed\n", __func__);
		goto bail;
	}
	...
	memcpy(buf, rfc7515_rsa_a1, l + 1);
	jws.args = jose_alg;
	jws.jwk = &jwk;
	jws.context = context;

	if (lws_jws_b64_concat_map(buf, strlen(buf), &jws.map_b64) != 3) {
		lwsl_notice("%s: lws_jws_b64_concat_map failed\n", __func__);
		goto bail;
	}

	/* overwrite the copy of the known b64 sig (it's all placed inside buf) */
	n = lws_jws_sign_from_b64(&jws, (char *)jws.map_b64.buf[LJWS_SIG],
	...
	buf[l] = '\0';
	if (lws_jws_sig_confirm_compact_b64(buf, l, &jwk, context) < 0) {
		lwsl_notice("%s: 2.2: confirm rsa sig failed\n", __func__);
		goto bail;
	}
...
test_jws_ES256(struct lws_context *context)
{
	...
	if (lws_jws_compact_decode(es256_cser, strlen(es256_cser),
				   &jws.map, &jws.map_b64, buf, sizeof(buf)) != 3) {
		lwsl_err("%s: concat_map failed\n", __func__);
		goto bail;
	}
	...
	/* parse the JOSE header */
	if (lws_jws_parse_jose(&jws.args, jws.map.buf[LJWS_JOSE],
				jws.map.len[LJWS_JOSE]) < 0) {
		lwsl_err("%s: JOSE parse failed\n", __func__);
		goto bail;
	}

		/* confirm we used "ES256" alg we expect from the JOSE hdr */
		if (strcmp(jws.args->alg, "ES256")) {
			lwsl_err("%s: JOSE header has wrong alg\n", __func__);
			goto bail;
		}
	...
	l = strlen(es256_cser);
	memcpy(buf, es256_cser, l + 1);

	/* scan the b64 compact serialization string to map the blocks */
	if (lws_jws_b64_concat_map(buf, l, &jws.map_b64) != 3)
		goto bail1;
...
test_jws_ES512(struct lws_context *context)
{
	...
	buf[l] = '\0';
	if (lws_jws_sig_confirm_compact_b64(buf, l, &jwk, context) < 0) {
		lwsl_notice("%s: confirm ecdsa sig failed\n", __func__);
		goto bail1;
	}
